...
php_versions=("/usr/local/php" "/opt/remi/php")

for php_version in "${php_versions[@]}"; do
    if [ -d "$php_version" ]; then
        for php_ini in "$php_version"/*/etc/php.ini; do
            if [ -f "$php_ini" ]; then
                echo "Updating PHP configuration for $php_ini"
                sed -i "s/^\(disable_functions\s*=\s*\).*\$/\1$disable_functions/" "$php_ini"
                sed -i "s/^\(upload_max_filesize\s*=\s*\).*\$/\1$upload_max_filesize/" "$php_ini"
                sed -i "s/^\(max_execution_time\s*=\s*\).*\$/\1$max_execution_time/" "$php_ini"
                echo "PHP configuration updated: $php_ini"
            else
                echo "PHP configuration file not found at $php_ini"
            fi
        done
    else
        echo "PHP path not found: $php_version"
    fi
done
